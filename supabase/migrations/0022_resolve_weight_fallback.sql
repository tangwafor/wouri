-- Wouri 0022: resolve documents against the allocated-lot weight when there is no
-- separate contract. The weight-consistency check still catches a real mismatch
-- whenever a contract exists; without one, the declared weight is the consignment
-- weight, so an operator can issue from allocated lots alone. No em-dashes.
...
begin
  ...
  -- No contract: the consignment weight is the declared weight (nothing to mismatch).
