...
export function OnboardingChat({ catalog }: { catalog: Cap[] }) {
  ...
  const labelOf = useMemo(() => {
    const m = new Map(catalog.map((c) => [c.capability_key, DEFAULT_LOCALE === 'fr' ? c.label_fr : c.label_en]));
  ...
  return (
    <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div>
        <label htmlFor="ws-desc">{t('onboarding')}</label>
        <textarea id="ws-desc" value={desc} onChange={(e) => setDesc(e.target.value)}
          rows={4} placeholder={t('describe_business')} style={{ resize: 'vertical' }} />
      </div>

      <div aria-live="polite">
        {detected.length ? (
          <>
            <div style={{ fontSize: '.82rem', color: 'var(--ink-3)', marginBottom: 6 }}>{t('aza_detected')}</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {detected.map((k) => <span className="pill on" key={k}>{labelOf(k)}</span>)}
            </div>
          </>
        ) : (
          <p style={{ margin: 0, color: 'var(--ink-3)', fontSize: '.85rem' }}>{t('aza_nothing')}</p>
        )}
      </div>

      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
        <button type="button" onClick={submit} disabled={!ready}>
          {busy ? t('setting_up') : t('setup_workspace')}
        </button>
        <span className="muted" style={{ marginTop: 0 }}>{t('or_click')}</span>
      </div>
