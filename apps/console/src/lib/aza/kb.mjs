...
export const APP = {
  ...
  provenance: {
    label_en: 'Document provenance', label_fr: 'Provenance des documents',
    what_en: 'A document is not a form with a branch per type. Each field is a binding row that names where its value comes from (the exporter record, the consignment, the commodity, a computed value like net weight or place of origin, a quality measurement), so adding a document type is data, not code. Every issued document records the source of each field, so anyone can ask which record produced any number on it. This is what makes the dependency graph, staleness, and cross-document checks possible.',
    what_fr: 'Un document n est pas un formulaire avec une branche par type. Chaque champ est une regle qui nomme d ou vient sa valeur (le dossier exportateur, l expedition, le produit, une valeur calculee comme le poids net ou le lieu d origine, une mesure de qualite), donc ajouter un type de document est une donnee, pas du code. Chaque document emis enregistre la source de chaque champ, si bien que l on peut demander quel enregistrement a produit chaque chiffre. C est ce qui rend possibles le graphe de dependances, la peremption et les controles entre documents.',
  },
  staleness: {
    label_en: 'Document staleness', label_fr: 'Peremption des documents',
    what_en: 'Because every issued document records the value of each field and its source (provenance), Wouri can tell when a document no longer matches reality. If someone amends a quantity, corrects a party, or re-grades a lot after a document was issued, that document is flagged stale, with the exact field, its old value, and its new one, so it can be reissued or revoked. Staleness is a comparison, not a rebuild, and an auto-check surfaces every stale document.',
    what_fr: 'Parce que chaque document emis enregistre la valeur de chaque champ et sa source (provenance), Wouri sait quand un document ne correspond plus a la realite. Si quelqu un modifie une quantite, corrige une partie ou reclasse un lot apres l emission, ce document est signale perime, avec le champ exact, son ancienne valeur et la nouvelle, pour etre reemis ou revoque. La peremption est une comparaison, pas une reconstruction, et une verification automatique fait remonter chaque document perime.',
  },
  geometry: {
    label_en: 'The server decides the plot', label_fr: 'Le serveur decide la parcelle',
    what_en: 'Origin geometry is a real geographic boundary, not a number the client types. When a plot boundary is drawn, the server computes the area, the centre, and whether the shape is valid; the number the operator typed is kept only as a declared, advisory value. A point is accepted only for a plot under the area cap (a registry figure). The registry then watches for a plot that overlaps another, a self-intersecting boundary, and a plot inside a protected area. This is the EUDR spine and the main anti-fraud surface.',
    what_fr: 'La geometrie d origine est une vraie limite geographique, pas un nombre saisi par le client. Quand une parcelle est tracee, le serveur calcule la surface, le centre et la validite de la forme; le nombre saisi par l operateur n est garde que comme valeur declaree, indicative. Un point n est accepte que pour une parcelle sous le plafond de surface (une valeur du registre). Le registre surveille ensuite une parcelle qui en chevauche une autre, une limite qui se recoupe et une parcelle dans une aire protegee. C est la colonne vertebrale EUDR et la principale surface anti-fraude.',
  },
  quota: {
    label_en: 'CITES quota (never negative)', label_fr: 'Quota CITES (jamais negatif)',
    what_en: 'A CITES-listed species has a national export quota per year. Every export debits it, and the balance can never go negative: a debit that would exceed the remaining quota is refused at the moment it is recorded, under a lock so two exports cannot both slip past. An exporter sees its own usage; the national remaining is shown as a total without revealing who exported what. Without this the CITES rail is not enforceable, and it is the moat.',
    what_fr: 'Une espece inscrite a la CITES a un quota national d export par an. Chaque export le debite, et le solde ne peut jamais devenir negatif: un debit qui depasserait le quota restant est refuse au moment de son enregistrement, sous verrou pour que deux exports ne passent pas ensemble. Un exportateur voit son propre usage; le restant national est un total sans reveler qui a exporte quoi. Sans cela la filiere CITES n est pas applicable, et c est l avantage decisif.',
  },
  mass_balance: {
    label_en: 'Mass balance', label_fr: 'Bilan de masse',
    what_en: 'A processing step (drying, milling, sorting) records what went in and what came out, and it cannot output more mass than it took in. That refusal is the control that stops laundering through a sawmill or a warehouse: you cannot turn a small legal input into a large one on paper. A step that loses mass (drying removes water) is normal and accepted; a step that gains mass is refused at write and flagged if it is ever written around the guard.',
    what_fr: 'Une etape de traitement (sechage, mouture, tri) enregistre ce qui entre et ce qui sort, et ne peut produire plus de masse qu elle n en a recu. Ce refus est le controle qui empeche le blanchiment par une scierie ou un entrepot: on ne peut pas transformer sur le papier une petite entree legale en une grande. Une etape qui perd de la masse (le sechage retire l eau) est normale et acceptee; une etape qui gagne de la masse est refusee a l ecriture et signalee si elle est ecrite en contournant la garde.',
  },
  groups: {
    label_en: 'Organization groups', label_fr: 'Groupes d organisations',
