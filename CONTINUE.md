...
- **Sprint 0 REMAINING (ungated):** apps/field (Expo); CI; the ui-qa-sweep browser e2e + Fabrice UAT gate. NOTE: local Next/Expo BUILD fails on this Windows box (corepack/symlink), so RUNTIME UI verify is via cloud (Netlify) or a Linux box, like akongne. tsc typecheck + the DB/app-path self-tests all pass locally.
