...
**ADR-0030. The Sprint 1/2 gate was overridden knowingly (2026-07-22).** ADR-0027 held Sprint 1 (the spine) and Sprint 2 (the document engine) until one real Cameroon consignment file was seen. The founder chose to proceed without it, accepting the reconciliation cost. Consequence and standing obligation: the spine is built to the architecture research (track 05), and the document engine to the documents research (track 03), NOT to a real file. When the first real consignment folder arrives, every field name, code list, weight/unit rule, and document layout in the spine and the document templates must be diffed against it and reconciled; treat mismatches as bugs, not enhancements. The riskiest surface is Sprint 2's Cameroon document set (EUR.1-CMR, phyto, VGM, quality certificate): its bindings are provisional until validated against a genuine issued document. This ADR does not delete ADR-0027; it records a deliberate, dated exception to it.
